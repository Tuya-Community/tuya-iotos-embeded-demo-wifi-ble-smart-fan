...
OPERATE_LIGHT opSocPwmInit(IN USHORT_T usFrequency, IN USHORT_T usPositiveDutyCycle, IN UCHAR_T ucChannelNum, IN UCHAR_T *pGpioList, IN BOOL_T bCCTFlag, IN BOOL_T bPolarity)
{
    ...
    if(bCCTDriveFlag) { /* CCT drive mode */
        for(i = 0; i < ucChannelNum; i++ ) {
            ...
            bk_pwm_initialize(pChannelList[i], uiPeriod, uiDuty, 0, 0);
        ...
        if(ucChannelNum == 2) {
            bk_cw_pwm_init(pChannelList[0], uiPeriod, uiDuty);
            bk_cw_pwm_init(pChannelList[1], uiPeriod, uiDuty);
            ...
            *(pPwmEnablelist + 1) = TRUE;
            ...
            bk_cw_pwm_init(pChannelList[3], uiPeriod, uiDuty);
            bk_cw_pwm_init(pChannelList[4], uiPeriod, uiDuty);
    ...
    if(bCCTDriveFlag) {    /* CCT drive mode */
        if(usDuty <= 0) {    /* duty <= 0 stop pwm, to reduce power */
            ...
            if( *(pPwmEnablelist + ucChannel) != TRUE ) {    /* channel pwm already stop, init pwm func again! */
				...
				bk_pwm_update_param(pChannelList[ucChannel], uiPeriod, fPercent * uiPeriod, 0, 0);
        ...
        if(ucChannelNumer == 2) {       /* CW light */
            if(ucChannel == 0) {
                ...
                if((uiCold == 0) && ((UINT_T)(fPercent * uiPeriod) == 0)) {
                    bk_cw_pwm_stop(pChannelList[0], pChannelList[1]);
                } else {
                    //PR_NOTICE("cold %d, warm %d", uiCold,(UINT_T)(fPercent * uiPeriod));
                    bk_cw_pwm_reset_duty_cycle(pChannelList[0], pChannelList[1], uiCold, (UINT_T)(fPercent * uiPeriod), uiPeriod, 0);
                }
            }
            ...
            if(ucChannel == 3) {      /* channel is C */
                ...
                if((uiCold == 0) && ((UINT_T)(fPercent * uiPeriod) == 0)) {
                    bk_cw_pwm_stop(pChannelList[3],pChannelList[4]);
                } else {
                    bk_cw_pwm_reset_duty_cycle(pChannelList[3], pChannelList[4], uiCold, (UINT_T)(fPercent * uiPeriod), uiPeriod, 0);
                }
            } else {
                if(usDuty <= 0) {    /* duty <= 0 stop pwm, to reduce power */
                    if( TRUE == *(pPwmEnablelist + ucChannel) ) {
